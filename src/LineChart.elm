...
> [view](#view) for visualizing _any_ amount of data series.</br>
> [line](#line) for configuring color, dot etc. of a line representing a data series.</br>
> [dash](#dash) for configuring color, dot etc. of a _dashed_ line representing a data series.</br>
...
viewCustom config lines =
    let
        ...
        dataSafe =
            List.map (List.filter .isReal) data

        dataAll =
            List.concat data

        dataAllSafe =
            List.concat dataSafe
...
viewBoxAttribute { frame } =
    Svg.Attributes.viewBox <|
        "0 0 "
            ++ String.fromFloat frame.size.width
            ++ " "
            ++ String.fromFloat frame.size.height
...
toDataPoints config lines =
    let
        ...
        addPoint datum =
            case ( x datum, y datum ) of
                ( Just x_, Just y_ ) ->
                    Just <| Data.Data datum (Data.Point x_ y_) True

                ( Just x_, Nothing ) ->
                    Just <| Data.Data datum (Data.Point x_ 0) False
    ...
    case config.area of
        ...
        Internal.Area.Normal _ ->
            data

        Internal.Area.Stacked _ ->
            stack data

        Internal.Area.Percentage _ ->
    ...
    let
        stackBelows dataset_ result =
            case dataset_ of
                data :: belows ->
                    stackBelows belows <|
                        List.foldl addBelows data belows
                            :: result
        ...
        iterate datum0 dataTop dataBelowTop result =
            case ( dataTop, dataBelowTop ) of
                ( datum1 :: data, datumBelow :: dataBelow ) ->
                    ...
                    if datum1.point.x > datumBelow.point.x then
                        ...
                        iterate datum1 data (datumBelow :: dataBelow) result

                ( [], datumBelow :: dataBelow ) ->
                    -- if the data point is after the point below, add it
                    if datum0.point.x <= datumBelow.point.x then
                        iterate datum0 [] dataBelow (add datumBelow datum0 :: result)
                        -- if not, try the next

                    else
                        iterate datum0 [] dataBelow (datumBelow :: result)
...
toSystem config data =
    let
        container_ =
            Internal.Container.properties identity config.container

        hasArea =
            Internal.Area.hasArea config.area

        size =
            Coordinate.Size (Internal.Axis.pixels config.x) (Internal.Axis.pixels config.y)

        frame =
            Coordinate.Frame container_.margin size
