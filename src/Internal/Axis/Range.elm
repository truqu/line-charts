module Internal.Axis.Range exposing (Config, applyX, applyY, custom, default, padded, window)
    ...
    case range of
        Padded padMin padMax ->
            let
                ...
                system_ =
                    { system | frame = { frame | size = { size | width = size.width - padMin - padMax |> Basics.max 1 } } }

                scale =
                    Coordinate.scaleDataX system_
            in
    ...
    case range of
        Padded padMin padMax ->
            let
                ...
                system_ =
                    { system | frame = { frame | size = { size | height = size.height - padMin - padMax |> Basics.max 1 } } }

                scale =
                    Coordinate.scaleDataY system_
            in
