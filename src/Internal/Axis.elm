...
import Svg exposing (Attribute, Svg, g, text, text_, tspan)
import Svg.Attributes as Attributes exposing (class, stroke, strokeWidth)
import Time
        ...
        , ticks =
            Ticks.custom <|
                \data range_ ->
                    let
                        smallest =
                            Coordinate.smallestRange data range_
        ...
        , ticks =
            Ticks.custom <|
                \data range_ ->
                    let
                        largest =
                            Coordinate.largestRange data range_
...
lengthOfTick { length, direction } =
    if Internal.Axis.Tick.isPositive direction then
        -length
...
viewHorizontalLabel system { direction, length } position view =
    ...
    in
    g
        [ transform [ move system position.x position.y, offset 0 yOffset ]
        , anchorStyle Middle
        ]
        [ view ]
...
viewVerticalLabel system { direction, length } position view =
    ...
    in
    g
        [ transform [ move system position.x position.y, offset xOffset 5 ]
        , anchorStyle anchor
        ]
        [ view ]
